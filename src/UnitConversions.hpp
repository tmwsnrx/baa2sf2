...
static constexpr int16_t adsr_ticks_to_timescents(uint16_t ticks)
{
...
constexpr int16_t volume_to_attenuation(float volume_multiplier)
{
...
constexpr int16_t volume_to_attenuation(uint16_t volume)
{
...
constexpr Tuning pitch_to_tuning(float pitch_multiplier)
{
...
constexpr int16_t pan_float_to_promille(float pan)
{
  if (pan <= -1.0f)
  {
    return -500;
